...
import { likeArticle, unlikeArticle } from '../../stores/slices/articleSlice';
...
const Article = ({ article }) => {
  ...
  const { slug, title, favoritesCount, tagList, author, createdAt, description } = article;
      ...
      <header className={styles['article__header']}>
        <div>
          <div className={styles['article__title-wrapper']}>
            ...
            <button className={styles['article__like-button']} onClick={handleLike}>
              <svg
                width="16"
                height="16"
                viewBox="0 0 16 16"
              ...
              >
                <path
                  ...
                  fill={article.favorited ? '#FF0707' : 'none'}
                />
                <path
                  ...
                  fill={article.favorited ? '#FF0707' : '#000000'}
                  fillOpacity={article.favorited ? '1' : '0.75'}
                />
              </svg>
            </button>
          ...
          <ul className={styles['article__tags']}>
            {tagList.map((tag, i) => (
              <li key={i} className={styles['article__tag']}>
                {tag}
              </li>
            ))}
          </ul>
        ...
        <div className={styles['article__info']}>
          <div>
            <div className={styles['article__username']}>{author.username}</div>
